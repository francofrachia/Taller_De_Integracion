...
// GET /api/direccion?id_usuario=X
...
const saveDireccion = async (req, res) => {
    try {
        const { id_usuario, calle, numero, id_localidad } = req.body;

        if (!id_usuario || !calle || !numero || !id_localidad) {
            return res.status(400).json({ error: 'Faltan campos obligatorios (id_usuario, calle, numero, id_localidad)' });
        }
